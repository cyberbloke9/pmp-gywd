...
class TeamSync {
   ...
   * @param {object} [config={}]
   * @param {string} [config.signingSecret] - HMAC secret (>=32 chars). If set, exports
   *   are signed and imports require a valid signature matching this secret.
   * @param {boolean} [config.requireSignature] - If true, importFromTeam rejects
   *   unsigned team data. Defaults to true if signingSecret is set.
   */
  constructor(globalMemory = null, config = {}) {
    ...
    if (config.signingSecret) {
      if (typeof config.signingSecret !== 'string' || config.signingSecret.length < 32) {
        throw new Error('TeamSync signingSecret must be a string of at least 32 chars');
      }
      this._signingSecret = Buffer.from(config.signingSecret, 'utf8');
      this._requireSignature = config.requireSignature !== false;
    } else {
      this._signingSecret = null;
      this._requireSignature = config.requireSignature === true;
    }
  }

  /** Canonical JSON for stable signing (sorted keys). */
  _canonicalize(obj) {
  ...
  /** Produce HMAC-SHA256 signature over the canonical payload. */
  _sign(payload) {
    if (!this._signingSecret) return null;
    const hmac = crypto.createHmac('sha256', this._signingSecret);
    hmac.update(this._canonicalize(payload));
    return hmac.digest('hex');
  }

  /** Verify an HMAC signature on a payload, returns true/false. */
  _verify(payload, signature) {
    if (!this._signingSecret) return false;
  ...
  exportForTeam(teamName, options = {}) {
    ...
    if (this._signingSecret) {
      exportData.signature = this._sign(exportData);
      exportData.signatureAlg = 'HMAC-SHA256';
    }
  ...
  importFromTeam(teamData, options = {}) {
    ...
    if (this._signingSecret) {
      if (!teamData.signature) {
        return { success: false, error: 'Team data is unsigned; refusing to import' };
      }
      // Verify against a copy with signature fields stripped
      const { signature, signatureAlg, ...payload } = teamData;
      if (signatureAlg && signatureAlg !== 'HMAC-SHA256') {
        return { success: false, error: `Unsupported signature algorithm: ${signatureAlg}` };
      }
      if (!this._verify(payload, signature)) {
        return { success: false, error: 'Signature verification failed' };
      }
    } else if (this._requireSignature && !teamData.signature) {
      return { success: false, error: 'Team data is unsigned; refusing to import (requireSignature=true)' };
    }
