...
class ProfileManager {
  ...
  /**
   * Get a compact profile for context-constrained scenarios
   * Returns only essential data to minimize token usage
   * @returns {object} Compact profile
   */
  getCompactProfile() {
    if (!this.profile) this.init();

    // Extract top preferences (high confidence only)
    const topPreferences = {};
    for (const [key, pref] of Object.entries(this.profile.preferences || {})) {
      if (pref && pref.confidence > 0.7) {
        topPreferences[key] = pref.value;
      }
    }

    // Get only recent high-value patterns
    const recentPatterns = (this.profile.patterns || [])
      .slice(-5)
      .map(p => p.type || p.description || 'unknown');

    return {
      version: this.profile.version,
      // Core cognitive style (essential for interaction)
      cognitive: {
        approach: this.profile.cognitive?.problemApproach,
        style: this.profile.cognitive?.debuggingStyle,
      },
      // Communication preferences (essential for output)
      communication: {
        verbosity: this.profile.communication?.verbosity,
        formality: this.profile.communication?.formality,
      },
      // Top expertise areas
      topExpertise: (this.profile.domain?.expertiseAreas || []).slice(0, 5),
      // Primary languages
      languages: (this.profile.tooling?.primaryLanguages || []).slice(0, 5),
      // High-confidence preferences
      preferences: topPreferences,
      // Recent pattern types
      recentPatterns,
      // Interaction stats (for context)
      stats: {
        sessions: this.profile.interactions?.totalSessions || 0,
        patterns: this.profile.patterns?.length || 0,
      },
    };
  }

  /**
   * Get profile size statistics
   * @returns {object} Size comparison
   */
  getProfileSize() {
    if (!this.profile) this.init();

    const fullJson = JSON.stringify(this.profile);
    const compactJson = JSON.stringify(this.getCompactProfile());
    ...
    return {
      full: fullSize,
      compact: compactSize,
      reduction: Math.round((1 - compactSize / fullSize) * 100),
      fullKB: (fullSize / 1024).toFixed(2),
      compactKB: (compactSize / 1024).toFixed(2),
    };
  }

  /**
   * Export profile for external use with size limit
   * @param {number} maxBytes - Maximum size in bytes
   * @returns {object} Profile data within size limit
   */
  exportWithLimit(maxBytes = 2048) {
    // Start with compact profile
    let exported = this.getCompactProfile();
    let json = JSON.stringify(exported);

    // If still too large, reduce further
    if (json.length > maxBytes) {
      exported = {
        cognitive: exported.cognitive,
        communication: exported.communication,
        topExpertise: exported.topExpertise?.slice(0, 3),
        languages: exported.languages?.slice(0, 3),
      };
    }
