...
class HookManager {
  ...
  register(type, handler, options = {}) {
    ...
    // Validate + pre-compile pattern to reject ReDoS-heavy regexes and
    // any syntactically invalid pattern before it's ever used at trigger time.
    let compiledPattern = null;
    if (options.pattern !== undefined && options.pattern !== null) {
      if (typeof options.pattern !== 'string') {
        throw new Error('Hook pattern must be a string');
      }
      if (options.pattern.length > 256) {
        throw new Error(`Hook pattern too long: ${options.pattern.length} chars (max 256)`);
      }
      if (this._isDangerousPattern(options.pattern)) {
        throw new Error(`Hook pattern rejected: contains catastrophic backtracking construct`);
      }
      try {
        compiledPattern = new RegExp(options.pattern);
      } catch (err) {
        throw new Error(`Hook pattern is invalid regex: ${err.message}`);
      }
    }
    ...
    const hook = {
      ...
      _compiledPattern: compiledPattern, // cached compiled regex
    };
   ...
   * Not exhaustive, but catches common ReDoS constructs.
   */
  _isDangerousPattern(pattern) {
    ...
    if (/\([^)]*\|[^)]*\)[+*]\{/.test(pattern)) return true;
    // Exponential backtracking via quantified group with quantifier inside
   ...
   */
  unregister(id) {
    for (const [_type, hooks] of this.hooks) {
  ...
  async trigger(type, context = {}) {
    ...
    for (const hook of hooks) {
      // Check pattern match (uses precompiled regex from register time)
      if (hook._compiledPattern && context.command) {
        // Truncate command to prevent pathological match on huge inputs
        const input = String(context.command).slice(0, 1024);
        if (!hook._compiledPattern.test(input)) {
          continue;
        }
      }
