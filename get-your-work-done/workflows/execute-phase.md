...
1. Create .planning/ISSUES.md if doesn't exist (use `~/.claude/get-your-work-done/templates/issues.md`)
...
See ~/.claude/get-your-work-done/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Use ~/.claude/get-your-work-done/templates/summary.md for structure.
...
For commit message conventions, see ~/.claude/get-your-work-done/references/git-integration.md
</step>
...
- options:
  - "Review issues" - Analyze with /gywd:consider-issues
  - "Continue" - Address later, proceed to next work

**If "Review issues" selected:**
- Invoke: `SlashCommand("/gywd:consider-issues")`
...
**In YOLO mode:**
- Note issues were logged but don't prompt: `📋 [N] issues logged this phase (review later with /gywd:consider-issues)`
...
`/gywd:execute-plan .planning/phases/{phase-dir}/{phase}-{next-plan}-PLAN.md`
...
**Also available:**
- `/gywd:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/gywd:verify-work {Z}` — manual acceptance testing before continuing
- `/gywd:discuss-phase {Z+1}` — gather context first
- `/gywd:research-phase {Z+1}` — investigate unknowns
...
**Also available:**
- `/gywd:verify-work` — manual acceptance testing before completing milestone
- `/gywd:add-phase <description>` — add another phase before completing
