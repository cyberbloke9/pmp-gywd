...
1. ~/.claude/get-your-work-done/templates/roadmap.md (milestone-grouped format)
...
- This contains context from `/gywd:discuss-milestone`
...
**If MILESTONE-CONTEXT.md exists (from /gywd:discuss-milestone):**
...
The main ROADMAP.md file holds full phase details for the active milestone. Archive files in `milestones/` are created only when a milestone ships (via `/gywd:complete-milestone`).
...
Plans:
- [ ] [N]-01: TBD (run /gywd:plan-phase [N] to break down)
...
This file was a handoff artifact from `/gywd:discuss-milestone`. Now that the milestone is created, the context is persisted in ROADMAP.md and the temporary file is no longer needed.
</step>
...
**Also available:**
- `/gywd:discuss-phase [N]` — gather context first
- `/gywd:research-phase [N]` — investigate unknowns
- Review roadmap
