...
2. Use template from `@~/.claude/get-your-work-done/templates/uat-issues.md`
3. Add each issue:
...
**Note:** Issues go to phase-scoped file, NOT global `.planning/ISSUES.md`. This keeps UAT findings tied to the specific work being tested and enables `/gywd:plan-fix` to address them.
</step>
...
- If issues: Suggest /gywd:plan-fix to address
```
</step>
...
If issues found:
- "Plan fixes" — Create plan to address issues (/gywd:plan-fix)
